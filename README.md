...
python3 gerber2nc.py <Board>
```

`gerber2nc.py` also prompts interactively for the base name if you run it with no arguments.
...
The `_visualization.png` produced by `gerber2nc.py` uses a KiCad-like palette, defined as `COLOR_*` constants on `OutputVisualizer`:
...
> **Note:** none of the `.nc`/`.gcode` files generated by either script contain bed-mesh (`M420`/`G29`) or Z-probing (`G30`/`M206`) commands — `gerber2nc.py`'s output always uses fixed Z heights, and no file ever explicitly enables or disables the mesh. Two independent corrections are at play, and it's worth not conflating them:
>
...
### Phase 2 — Milling and drilling (`gerber2nc.py` output)
