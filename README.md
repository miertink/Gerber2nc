...
> **Note:** none of the `.nc`/`.gcode` files generated by either script contain bed-mesh (`M420`/`G29`) or Z-probing (`G30`/`M206`) commands themselves — `gerber2nc_v2.py`'s output always uses fixed Z heights. If bed-mesh compensation is in effect during milling, it's only because it's still active in the firmware from Phase 1 (`M420 S1` is only ever emitted in the `ContourMapping` file). Z-referencing before drilling (steps below) is always a manual step you type at the machine's console — it isn't generated by any script.
...
2. **Run `<Board>_Engraving.nc`** — isolates all copper (traces, pads, zone fills). Benefits from the bed-mesh compensation set up in Phase 1 only if it's still active in the firmware — the file itself uses fixed Z heights.
